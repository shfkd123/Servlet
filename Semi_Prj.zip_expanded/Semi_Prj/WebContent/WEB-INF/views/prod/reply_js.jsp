...
window.onload=function(){
	getPage("<%=request.getContextPath()%>/reply/list.do?id=${prod.id}&page="+replyPage);
	...
	$.ajax({
		...
		success:function(data){
			...
			if(result[0]=="SUCCESS"){
				...
				getPage("/reply/list.do?id="+id+"&page="+result[1]); //리스트 출력
				$('#newReplyText').val("");				
			}else{
...
function replyRemove_go(){
	...
	$.ajax({
		url : "<%=request.getContextPath()%>/reply/remove.do?rno="+rno+"&page="+replyPage+"&id=${prod.id}",
		type:"get",
		success:function(page){
			alert("삭제되었습니다.");
			getPage("<%=request.getContextPath()%>/reply/list.do?id=${prod.id}&page=" + page );
			replyPage = page;
		},
		error:function(error){
			alert('삭제에 실패했습니다.');

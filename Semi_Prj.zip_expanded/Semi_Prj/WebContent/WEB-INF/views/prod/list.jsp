				...
				<div id="keyword" class="card-tools" style="width: 450px;">
					<div class="input-group row">
						<select class="form-control col-md-3" name="perPageNum"
							...
							id="searchType">
							<option value="ndc" ${cri.searchType eq 'ndc' ? 'selected':'' }>전
								체</option>
							<option value="n" ${cri.searchType eq 'n'   ? 'selected':'' }>상품명</option>
							<option value="d" ${cri.searchType eq 'd'   ? 'selected':'' }>내
								용</option>
							<option value="c" ${cri.searchType eq 'c'   ? 'selected':'' }>카테고리</option>
							<option value="nc" ${cri.searchType eq 'nc'  ? 'selected':'' }>상품명+카테고리</option>
							<option value="dc" ${cri.searchType eq 'dc'  ? 'selected':'' }>내
								용+카테고리</option>
							<option value="ncd" ${cri.searchType eq 'ncd' ? 'selected':'' }>상품명+내
								용+카테고리</option>
